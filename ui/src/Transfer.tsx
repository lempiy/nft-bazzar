...
export const Transfer: FC<ITransferProps> = ({ nft, id, onTransferred }) => {
  const [state, setState] = useState<ITransferState>({
    pickedUserID: "",
  ...
  const transfer = async () => {
    ...
    const toATA = account.value.length
      ? account.value[0].pubkey
      : await createNewATA(toWallet, mintKey);
    ...
    updateDoc(docRef, {
      owner: state.pickedUserID,
      account_id: toATA.toBase58(),
    });
        ...
        <Grid item xs={2} className="text-center justify-center stretch">
          <div className="transfer-holder">
            <IconButton
              ...
              size="large"
              disabled={!state.pickedUserID || state.isTransferring}
            >
